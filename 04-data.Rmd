...
The variables you can use in a model will always be limited to those that are available, so sometimes the first task in this CRISP-DM phase really is as simple as "get data." In the case of the MegaTelCo project, let us suppose, the data set for the analysis has already been  curated by Marketing and includes every data element collected by the company that might reasonably be associated with---or could conceivably explain---customer retention.

Here is the data dictionary for the data set you received from Marketing.
...
Let's import that data set, `megatelco.csv`, making sure also to load the packages we'll need. (This data set is available on Canvas.)

<!-- Note that this chapter is written as if we were working within an R Markdown document with code chunks. You are welcome to copy and paste the code from each chunk into your own R Markdown document in RStudio.   -->
<!-- The `megatelco.csv` data set used in this chapter is available on Canvas. -->
...
# Load data from file into the environment
m <- read.csv("megatelco.csv")


```

This code imports `megatelco.csv` from the working directory and assigns it to an object, `m` (for "MegaTelCo"), stored in the environment. I like to keep the names of data objects short to reduce typing. That is a matter of personal style.


For this code to work on your machine the file you are loading must be in the working directory.  Which folder is that?  You can check with `getwd()`. In general, I find it easiest to locate a downloaded file like `megatelco.csv` in my downloads folder, and then manually copy and paste it to my working directory. Then the above code will work because the file is located where the function expects it.
...
The next step is to look at the data and  make sure that  it has loaded correctly. Use the `str()` function.
...
```
There are two kinds of variables in this data set: character (denoted `chr`) and numeric (`int`).
...
- factor variables ensure that plots work as expected.

For example, `reported_satisfaction`  has the three unique values: `low,` `avg,` `high.` Because this is a character variable, these categories are not ordered, but they obviously should be: `low` < `avg` < `high.`  We can encode this ordering by transforming the variable into an *ordinal* factor.^[Note that it is not always necessary to convert character variables into factor variables manually as part of the data cleaning process.  Some modeling algorithms, such as linear regression, will do the conversion automatically.]

Additionally, there are some mistaken values in the data set. `income` and `house` both represent dollar values. Thus negative values should not be regarded merely as outliers but as errors.  `handset_price` has a max that  is unrealistically high. And `over_15mins_calls_per_month`  has three missing observations marked as `NA`.
...
The mean or average of a sample of data will be familiar to you: add up the observations and divide by the total number of observations.  In statistics the mean of a sample is referred to as $\bar{x}$, pronounced "ex-bar."  Here is the formula:  
...
```{r}
numbers <- c(1, 2, 4, 6, 8, 10)
summary(numbers)
```

Let's look closer at the calculation.

Here is the mean:

$$
\bar{x}=\frac{x_1+x_2+\cdots+x_n}{n}=\frac{1+2+4+6+8+10}{6}=\frac{31}{6}=5.166667
$$
And here is the median:

$$
\text{median}(x)=x_{(n+1)/2}=x_{(6+1)/2}=x_{3.5}
$$
$x_{3.5}$ represents the number that is midway between the third and fourth numbers---in this case, between 4 and 6---which is 5.
...
So far we have discussed measures of central tendency. But any statistic measuring the center will be less representative as the data becomes more variable.  That is why, in summarizing data, we focus on measures of spread or variability, along with measures of central tendency. Standard deviation is probably the best known measure of variability.  It represents the average deviation of each observation from the mean of the data. In statistics, the standard deviation of a sample of data is represented as $s$.  The calculation of $s$ is a little tricky.  Here is the formula.
...
<!-- (I leave it to you to puzzle through this code. The first line will be unfamiliar.  We need to turn the vector of observations into a data frame for use with dplyr.  For that we use the `data.frame()` function.) How accurate is this calculation?  We can double check using the R function `sd()`: -->
...
Here is a Kahn academy video on measures of spread, including standard deviation.
...
- Remove rows with missing values (`NA`) in `over_15mins_calls_month`. Dealing with missing values can be a complicated problem.  In this case, because the number of missing values is small relative to the size of the data set, it is fine to solve the problem by removing rows.
...
In base R, the `factor()` function will create a factor.  In doing so we need to specify the factor structure, otherwise R will automatically assign levels based on alphabetic (or numeric) order, which in this case is inappropriate and will cause problems, for example, when plotting. Here is the basic syntax: `factor(x, levels, ordered)`.
...
Let's now specify the order of the levels to be different from the default alphabetic ordering.

```{r}
factor(x = f, levels = c("z", "y", "x"))
```

What happens if we add `ordered = T`? Now the levels are not merely ordered in the sense that the alphabet is ordered, with a sequence; they are ordered in the sense that numbers are ordered: each level is greater than or less then the other levels.

```{r}
factor(x = f, levels = c("z", "y", "x"), ordered = T)
```


So, we need to change the factor structures of `reported_statisfaction`, `reported_usage_level`, `considering_change_of_plan`, and `leave`.  We could do that to each variable separately, but it is more efficient (and less error prone) to make the changes in a single dplyr code chunk:
...
m %>%
  mutate(reported_satisfaction = factor(reported_satisfaction,
         ...
         leave = factor(leave)) -> m_clean 
```

Changes to the data have been stored in a new data set, `m_clean`, in the final line.  

Here is an explanation of the above code:

1. ` m %>%`: The pipe operator, which can be read as "and then." Note that "command-shift-m" is a shortcut for `%>%`. This code pipes the data frame into the first argument of the next line.
2. `mutate`:  This is `dplyr` command for creating a new variable.  In this case we will not be creating brand new variables, but overwriting old ones, replacing the existing character variables with factor variables.
3. `reported_satisfaction = factor(reported_satisfaction, levels = c("low","avg", "high"), ordered = T)`: Overwrite the existing `reported_satisfaction` character variable with an ordered factor using the specified level structure. The `levels()` argument allows you to set the order of the levels to override the default alphabetic ordering. The `ordered()` argument allows you to specify whether one level should be considered greater than another.
4. `reported_usage_level = factor(reported_usage_level, levels = c("low",  "avg", "high"), ordered = T)`: Same explanation as above.
5. `considering_change_of_plan = factor(considering_change_of_plan, levels = c("no", "maybe", "yes")`: Same explanation as above except that we have omitted the `ordered()` argument, relying instead on the default `ordered = F` argument.
6. `leave = factor(leave)) -> m_clean`: Same explanation as above except that now we will use the defaults only: alphabetic level ordering (first `LEAVE` then `STAY`) and `ordered = F`. The data frame that results from all the changes above is stored, via the assignment operator, in a new data frame, `m_clean.` (We could do the assignment at the beginning, `m_clean <- ...`, but doing it at the end better fits with the top-down, left-right flow of dplyr
...
m_clean %>%
  mutate(college = ifelse(college == "one", 1, 0)) -> m_clean
...
1. `m_clean %>%`: Can be translated as "use `m_clean` as the source data and then ...." Here we are not saving the results of this code but simply printing it for information.
...
4. `head`:  Filters the data set for just the top six observations.
...
1. `m_clean  %>% `: Use `m_clean` and then.
2. `filter(income > 0, house > 0, handset_price < 1000)) -> m_clean`: Subset the data based on logical conditions. Here we know that income must be positive, home prices must be positive, and that the price of the phone should be greater than the second highest---and legitimate---value in the data set: 899. So we arbitrarily chose 1000 as the cutoff. Remember that `filter()` subsets a data set by rows, using logical conditions, while `select()` subsets a data set by columns, using column names. Finally, we overwrite the existing data set in order to save the changes.

### Remove NAs

To remove the NAs in `over_15mins_calls_per_month` we will use the `na.omit()` function, which does just what it says:  it omits any row in the data set containing an `NA.` Remember that there is no way to remove individual missing observations; the entire row must be removed. `na.omit()` therefore performs a filtering operation known as "row-wise deletion."


```{r ex13_5}
# Remove NAs
m_clean <- na.omit(m_clean)
```

For such a small change we can use nested syntax without loss of clarity.  
...
We have removed 6 rows in all and the data set should now be clean and ready for exploration.  
...
For purposes of illustration, we will focus on just a couple of the demographic variables here.
...
<!-- ```{r} -->
<!-- # Boxplot of income -->
<!-- ggplot(data = m_clean, mapping = aes(y = income)) + -->
<!--   geom_boxplot()+ -->
<!--   labs(title = "Boxplot") -->
<!-- ``` -->
...
<!-- Above we used a boxplot with just a single variable, but, more typically, a boxplot is used to show the relationship between a continuous variable and categorical variable.  -->

Here is example code for creating a boxplot comparing `income` at the two levels of `leave`. 

```{r ex17}
# Boxplot of leave vs. income
ggplot(data = m_clean, 
       mapping = aes(x = leave, y = income)) +
  geom_boxplot() +
...
1. `ggplot(data = m_clean, mapping = aes(x = leave, y = income))`:  Creates a blank ggplot canvas with `m_clean` as the data source, and `leave` on the x-axis and `income` on the y-axis. Notice that we are not saving this visualization in an object, but rather just printing it for information.
...
What about `college`? To plot this relationship we cannot use a boxplot.  Why?  Both `college` and `leave` are binary, but a boxplot shows the distribution of a *numeric* variable at different levels of a *categorical* variable.  Instead, we need a plot type that will allow us to compare two *categorical* variables. A bar plot is a good choice.   The creation of the bar plot in this instance, with two binary variables, is a little tricky, however. We need to do some data manipulation before creating the plot by calculating the number of observations, $n$, for each unique combination `leave` and `college`. 
...
m_clean %>%
  count(leave, college) %>%
  ggplot(mapping = aes(x = college, y = n, fill = leave)) +
  geom_col() +
...
2. `ggplot(mapping = aes(x = college, y = n, fill = leave))`: Create a blank ggplot canvas with `college` on the x-axis, `n` on the y-axis and a third variable, `leave`, as the fill variable. The `fill` argument indicates that the bars should be colored by the two `leave` categories. Notice that the data argument has been omitted. This works because `ggplot` will automatically use the data that is being piped in.
...
m_clean %>%
  count(leave, college) %>%
  group_by(college) %>% # group_by "college" to calculate the proportion of churn in each level
...
m_clean %>%
  ...
  group_by(college) %>%
  mutate(proportion = n / sum(n)) %>%
  ggplot(mapping = aes(x = college, y = proportion, fill = leave)) +
  geom_col(position = "dodge") +
  labs(title = "leave ~ college")
...
m_clean %>%
  ...
  ggplot(aes(x = college, y = proportion)) +
  geom_col() +
  labs(title = "leave ~ college")
  
```

This code should all be familiar---it has been discussed above--- with the exception of `mean(leave=="LEAVE")`.  Why does this calculate the proportion of leavers within the two levels of `college`?  It exploits the fact that R works with a logical vector---which is what `leave=="LEAVE"` produces (answering the question, "does `leave` have the value of`LEAVE`?", with `T` or `F`)---as a 0/1 vector.  Thus, we can take the mean, which returns the proportion of 1s.

However, this bar plot doesn't work very well.  While it captures the same information as the earlier plot, is misleading: the proportion for `STAY` is missing.  For this reason the earlier plot, with bars for both `STAY` and `LEAVE`, is preferable.
...
We have skipped over exploration of behavioral and attitudinal variables in the interest of brevity.  There is always more EDA to do!   In fact, because the phases in the CRISP-DM model  are  iterative and reversible, later phases in a project, such as modeling and model evaluation/deployment,  will often necessitate more EDA.  

<!-- For now, let's summarize what we have learned.   In some cases our initial hypotheses were supported, in other cases not. -->
...
- `summary()`. Summarizes the variables in a data set. The output is especially useful for getting a quick understanding of the distribution of numeric variables. It reports: min, max, first quartile, third quartile, median and mean. For factor variables the function reports the number of observations at each level. 
- `==`. Evaluates logically, returning TRUE if two items are identical, otherwise FALSE.
- `factor()`. Creates a factor variable.
- `na.omit()`. Removes all rows with NAs from the data set.
...
- `str()`. Provides a snapshot view of a data set.
...
- `facet_wrap()`.  Creates a small multiples plot consisting in plots for different levels of a variable.
...
- `Exploratory data analysis or EDA`. An approach to analyzing a data set, often preparatory to modeling, that involves discovering patterns and summarizing its main characteristics, often using visualization.  
