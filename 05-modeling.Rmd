...
We will follow DSB in using log base 2 to calculate entropy and information gain.  Log base 2 is used, rather than natural log (which is more common in statistics), because it ensures that entropy scales between 0 and 1. (Natural log would work just fine for calculating entropy for comparative use but it would not have this nice property.)
...
For future calculations we will not round the inputs since then our final result would be thrown off by rounding error.
